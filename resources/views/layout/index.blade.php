...
<body>
  ...
  <div class="custom-nav">
    <div class="nav-row">
      ...
      <a class="nav-link {{Request::is('activities') ? 'selected underlined': ''}}" aria-current="page" href="/activities">Aktivitas</a>
    ...
    <div class="nav-row">
        ...
        @if(auth()->user()->role->name == "Sukarelawan")
            <img class="custom-test-profile-image shadow-4-strong" src={{ asset('storage/' . auth()->user()->sukarelawan->profileImageUrl) }} alt="sukarelawan image">
        @elseif (auth()->user()->role->name == "Fasilitator")
            <img class="custom-test-profile-image shadow-4-strong" src={{ asset('storage/' . auth()->user()->fasilitator->logoImageUrl) }} alt="fasilitator image">
        @endif
